...
class Pelt:
    ...
    "scent": ["algae", "almonds", "aloe", "anise", "ants", "apples", "apricot", "ash", "asters", "baked beans", "bananas", "bark", "basil", "batteries", "beach air", "beef", "beeswax", "berries", "betony", "bindweed", "birch sap", "black pepper", "blackberries", "blood", "bluebells", "blueberries", "bone marrow", "bread", "broom", "burnet", "butter", "canned tuna", "caramel", "cardamom", "cardboard", "carrots", "casserole", "cat breath", "cat food", "cat treats", "catnip", "cayenne pepper", "celandine", "chamomile", "cheese", "cherries", "cherry blossoms", "chervil", "chicken nuggets", "chickweed", "chili pepper", "chocolate", "cilantro", "cinnamon", "citrus", "clay", "clean fur", "clover", "cobwebs", "coconut", "coffee", "cold air", "coltsfoot", "comfrey", "concrete", "copper", "corn", "cornflowers", "crabapple", "crocuses", "crowfood", "curry", "daffodils", "dahlias", "daisies", "dandelions", "daylilies", "dead leaves", "dill", "dirt", "dock", "dog", "dogwood", "dust", "eggnog", "eggs", "eucalyptus", "feathers", "fennel", "fire", "fish", "fish sticks", "flowers", "fog", "forests", "forget-me-nots", "fossils", "foxgloves", "fried chicken", "fried onions", "fries", "frost", "gardenias", "garlic", "gelatin", "ginger", "glazed ham", "goatweed", "goldenrod", "grapefruit", "grass", "gravel", "gravy", "green beans", "hailstones", "hamburgers", "hashbrowns", "hawkweed", "hay", "heather", "heliotropes", "hemlock", "hollies", "honey", "honeysuckle", "horsetail", "hot air", "hot dogs", "hot sauce", "hummus", "hyacinths", "icicles", "jasmine", "laurel", "lavendar", "lavender", "leather", "leaves", "lemongrass", "lettuce", "lilies", "lint", "locust", "magnolias", "mallow", "maple syrup", "marigold", "marinara sauce", "marjoram", "marsh water", "mayonnaise", "metal", "mildew", "milk", "mist", "moss", "mozzarella", "mud", "mustard", "nectar", "nightshades", "noodles", "nutmeg", "oatmeal", "oats", "oil", "olive oil", "olives", "onions", "orange blossom", "oranges", "oregano", "osmanthus", "oysters", "ozone", "paint", "pansies", "paper", "paprika", "parmesan cheese", "parsley", "peaches", "peanut butter", "pears", "pecan pie", "peppermint", "pine needles", "pine sap", "pineapple", "pinecone", "plastic", "pollen", "popcorn", "poppies", "potato chips", "potatoes", "prime rib", "pumpkin", "ragweed", "ragwort", "rain", "raspberries", "raw chicken", "reeds", "rhubarb", "rice", "rocks", "rosemary", "roses", "rotten fish", "rue", "rust", "saffron", "sage", "salad dressing", "salmon", "salt", "saltwater", "sand", "sesame", "sharp cheddar", "shells", "shrimp", "silk", "smoke", "snow", "snowdrops", "soap", "sorrel", "soybeans", "spearmint", "spinach", "static electricity", "steel", "stinkbug", "storms", "strawberries", "sugar", "sulfur", "sunflowers", "sushi", "sweet peas", "tansy", "tapioca pudding", "tar", "tea", "thyme", "toast", "tomatoes", "toothpaste", "tormentil", "tree sap", "trees", "truffles", "tulips", "tumeric", "turkey", "turkey stuffing", "twigs", "twolegs", "undergrowth", "vanilla", "varnish", "vinegar", "violets", "walnuts", "wasabi", "water", "watermelon", "whipped cream", "willows", "wintergreen", "wires", "wood", "wool", "yew"],
    ...
    blue_eyes = sprites.eye_colors["color_lists"]["blue_eyes"]
    ...
    natural_blue_eyes = sprites.eye_colors["natural_colors"]["blue_eyes"]
    natural_green_eyes = sprites.eye_colors["natural_colors"]["green_eyes"]
    natural_red_eyes = sprites.eye_colors["natural_colors"]["red_eyes"]
    ...
    tortie_patterns = [tabbies, spotted, plain, exotic]
    ...
    def check_and_convert(self, convert_dict, extra_traits, species_traits):
        ...
        # convert tortie
        if self.tortie_base == "single":
            self.tortie_base = "singlecolour"
        ...
        if self.tortie_marking in convert_dict["old_tortie_patches"]:
            ...
            # If the tortie_marking is old, there is also a chance the base color is stored in
    ...
    def init_eyes(self, parents):
        ...
        if switch_get_value("cur_screen") == "make clan screen":
            natural = game_setting_get("natural_colors")
        else:
            natural = get_clan_setting("natural_colors_clan")

        if natural:
            if not parents:
                self.eye_colour = choice(Pelt.natural_eye_colours)
            else:
                self.eye_colour = choice(
                    [i.pelt.eye_colour for i in parents] + [choice(Pelt.natural_eye_colours)]
                )
        else:
            if not parents:
    ...
    def pattern_color_inheritance(self, parents: tuple = (), gender="female"):
        ...
        if torbie:
            ...
            if chosen_tortie_base.upper() in ("TWOCOLOUR", "SINGLECOLOUR"):
        ...
        if chosen_pelt.upper() in ("TWOCOLOUR", "SINGLECOLOUR", "SINGLE"):
            tortie_marking = sprites.pelt_colors["pelt_list"]["SINGLECOLOUR"]["colors"]
        elif chosen_pelt in ("Calico", "Tortie"):
            if chosen_tortie_base.upper() in ("TWOCOLOUR", "SINGLECOLOUR", "SINGLE"):
                tortie_marking = sprites.pelt_colors["pelt_list"]["SINGLECOLOUR"]["colors"]
            else:
                tortie_marking = sprites.pelt_colors["pelt_list"][chosen_tortie_base.upper()]["colors"]
        else:
            tortie_marking = sprites.pelt_colors["pelt_list"][chosen_pelt.upper()]["colors"]

        weights = [0, 0, 0, 0]
        for p_ in par_peltcolours:
            if p_ in sprites.pelt_colors["color_list"][tortie_marking]["categories"]["ginger_colors"]:
                add_weight = (40, 0, 0, 10)
            elif p_ in sprites.pelt_colors["color_list"][tortie_marking]["categories"]["black_colors"]:
                add_weight = (0, 40, 2, 5)
            elif p_ in sprites.pelt_colors["color_list"][tortie_marking]["categories"]["white_colors"]:
                add_weight = (0, 5, 40, 0)
            elif p_ in sprites.pelt_colors["color_list"][tortie_marking]["categories"]["brown_colors"]:
            ...
            if all([x == 0 for x in weights]):
                weights = [1, 1, 1, 1]
        pelt_choices = []
        for category in sprites.pelt_colors["color_list"][tortie_marking]["categories"].values():
            pelt_choices.append(category)
        ...
        if chosen_pelt.upper() in ("TWOCOLOUR", "SINGLECOLOUR"):
            if chosen_white:
    ...
    def randomize_pattern_color(self, gender):
        ...
        if chosen_pelt.upper() in ("TWOCOLOUR", "SINGLECOLOUR"):
            ...
            if chosen_tortie_base in ("TwoColour", "SingleColour", "single"):
                tortie_marking = sprites.pelt_colors["pelt_list"]["SINGLECOLOUR"]["colors"]
            else:
                tortie_marking = sprites.pelt_colors["pelt_list"][chosen_tortie_base.upper()]["colors"]
        else:
    ...
    def init_pattern(self):
        if self.name in Pelt.torties:
            if not self.tortie_base:
                self.tortie_base = choice(Pelt.tortie_patterns)
            if not self.tortie_marking:
                self.tortie_marking = choice(Pelt.tortiemasks)
            ...
            if self.colour:
                ...
                if not wildcard_chance or random.getrandbits(wildcard_chance) == 1:
                    ...
                    # Allow any tortie_marking:
                    self.tortie_pattern = choice(random.choices(Pelt.tortie_patterns, k=1)[0])
                    # Allow any colors that aren't the base color.
                    if self.tortie_pattern in ("TwoColour", "SingleColour", "single"):
                        tortie_marking = sprites.pelt_colors["pelt_list"]["SINGLECOLOUR"]["colors"]
                    else:
                        tortie_marking = sprites.pelt_colors["pelt_list"][self.tortie_pattern.upper()]["colors"]
                    
                    possible_colors = []
                    for category in sprites.pelt_colors["color_list"][tortie_marking]["categories"].values():
                        possible_colors.append(category)
                    if self.colour in possible_colors:
                        possible_colors.remove(self.colour)
                    self.tortie_colour = choice(random.choices(possible_colors, k=1)[0])

                else:
                    
                    # Normal generation
                    if self.tortie_base in ("Singlestripe", "Smoke", "SingleColour", "TwoColour"):
                        self.tortie_pattern = choice(
                            [Pelt.tabbies, Pelt.spotted]
                        )
                    else:
                    ...
                    # get tortie_marking
                    if self.tortie_pattern.upper() in ("TwoColour", "SingleColour", "single"):
                        tortie_marking = sprites.pelt_colors["pelt_list"]["SINGLECOLOUR"]["colors"]
                    else:
                        tortie_marking = sprites.pelt_colors["pelt_list"][self.tortie_pattern.upper()]["colors"]

                    colors = sprites.pelt_colors["color_list"][tortie_marking]["categories"]

                    if self.colour == "WHITE":
                        possible_colors = list(colors["white_colors"].copy())
                        possible_colors.remove("WHITE")
                        self.colour = choice(possible_colors)

                    # Ginger is often duplicated to increase its chances
                    if (self.colour in colors["black_colors"]) or (
                        self.colour in colors["white_colors"]
                    ):
                        self.tortie_colour = choice(
                            (colors["ginger_colors"] * 2) + colors["brown_colors"]
                        )
                    elif self.colour in colors["ginger_colors"]:
                        self.tortie_colour = choice(
                            colors["brown_colors"] + colors["black_colors"] * 2
                        )
                    elif self.colour in colors["brown_colors"]:
                        possible_colors = list(colors["brown_colors"].copy())
                        possible_colors.remove(self.colour)
                        possible_colors.extend(
                            colors["black_colors"] + (colors["ginger_colors"] * 2)
                        )
    ...
    def init_tint(self):
        ...
        if switch_get_value("cur_screen") == "make clan screen":
            ...
            natural = get_clan_setting("natural_colors_clan")
        if natural:
            base_tints = sprites.cat_tints["natural_possible_tints"]["basic"]
            if self.colour in sprites.cat_tints["colour_groups"]:
                color_group = sprites.cat_tints["colour_groups"].get(self.colour, "warm")
                color_tints = sprites.cat_tints["natural_possible_tints"][color_group]
            else:
                color_tints = []
...
def _describe_torties(cat, color_name, short=False) -> [str, str]:
    ...
    # get tortie_marking
    if cat.pelt.tortie_pattern.upper() in ("TWOCOLOUR", "SINGLECOLOUR"):
        tortie_marking = sprites.pelt_colors["pelt_list"]["SINGLECOLOUR"]["colors"]
    else:
        tortie_marking = sprites.pelt_colors["pelt_list"][cat.pelt.tortie_pattern.upper()]["colors"]
    ...
    if short:
        ...
        if (
            cat.pelt.colour
            in colors["black_colors"] + colors["brown_colors"] + colors["white_colors"]
    ...
    if (
        cat.pelt.colour in colors["black_colors"] + colors["brown_colors"] + colors["white_colors"]
        and cat.pelt.tortie_colour
        in colors["black_colors"] + colors["brown_colors"] + colors["white_colors"]
    ):
